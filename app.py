...
def anomalies():
    ...
    try:
        ...
        for i in range(1, len(usage_data)):
            ...
            # Remove the consecutive date check to allow comparisons across any gap
            # Check for anomaly based on units change
            units_change = abs(current['units'] - previous['units'])
            units_threshold = previous['units'] * 0.20  # 20% threshold
            ...
            # Check for anomaly based on bill change
            bill_change = abs(current['bill'] - previous['bill'])
            bill_threshold = previous['bill'] * 0.20  # 20% threshold
